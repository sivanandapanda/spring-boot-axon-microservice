...
public class OrdersServiceErrorHandler {
    ...
    public ResponseEntity<Object> handleIllegalStateException(IllegalStateException ex, WebRequest webRequest) {
        var errorMessage = new ErrorMessage(ex.getMessage(), new Date());
    ...
    public ResponseEntity<Object> handleCommandExecutionExceptions(CommandExecutionException ex, WebRequest webRequest) {
        var errorMessage = new ErrorMessage(ex.getMessage(), new Date());
        return new ResponseEntity<>(errorMessage, new HttpHeaders(), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}